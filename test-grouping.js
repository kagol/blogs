/**
 * 测试脚本：验证按月份分组功能
 * 运行方式：node test-grouping.js
 */

import path from 'path'
import { fileURLToPath } from 'url'
import { scanArticlesInDirectory, generateMonthlyGroupedSidebar } from './docs/.vitepress/utils/article-grouping.js'
...
console.log('\n文章列表（按时间从近到远）：')
openSourceArticles.forEach((article, index) => {
  console.log(`${index + 1}. [${article.year}-${String(article.month).padStart(2, '0')}] ${article.title}`)
  console.log(`   路径: /${article.path}`)
})
